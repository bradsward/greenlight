...
def test_failure_messages() -> None:
    ...
    # Not pinning the exact wording -- it's generated by the installed
    # `mcp` SDK from boom()'s exception, and that format isn't stable
    # across SDK versions (see test_proxy_e2e.py and
    # test_tool_error_extraction.py for why). Just checking the message
    # made it through to both the dict and the printed report at all.
    path = asyncio.run(_record_boom_session())
    stats = compute_stats(path)
    assert stats["tool_errors"] == 1
    assert len(stats["failure_messages"]) == 1
    assert "boom" in stats["failure_messages"][0]

    report = format_stats(stats, path)
    assert stats["failure_messages"][0] in report
